...
extern int wfd; //ipython3 parent process writer
...
void ex_test(exarg_T *eap)
{
  //get the number of the line ###===n
  win_T *curwin_dummy = (win_T*)malloc(sizeof(win_T));
  memcpy(curwin_dummy, curwin, sizeof(win_T));
  buf_T *curbuf_dummy = (buf_T*)malloc(sizeof(buf_T));
  memcpy(curbuf_dummy, curbuf, sizeof(buf_T));
  pos_T pos_n; //This will show on which line "###===n" is
  pos_n.lnum = 1; pos_n.col = 0; pos_n.coladd = 0;
  
  int numofcell = atoi((char*)eap->arg);
  char nofcellstr[256];
  snprintf(nofcellstr, 256, "%d", numofcell + 1);
  int i;
  for(i=0; nofcellstr[i] != '\0'; ++i);
  char_u searchstr[256]; //make a string "###===n" to search for
  int jend = 6;
  for(int j=0; j < 3; j++)
    searchstr[j] = '#';
  for(int j=0; j < 3; j++)
    searchstr[j+3] = '=';
  for(int j=0; j < i; j++){
    searchstr[j+6] = nofcellstr[j];
    jend++;
  }
  
  searchstr[jend] = '\0';

  int isfound = searchit(curwin_dummy, curbuf_dummy, &pos_n,
    NULL, FORWARD, searchstr,
    1, 12, 2, (linenr_T)0, NULL, NULL);
  if(isfound == 0)
    pos_n.lnum = curbuf->b_ml.ml_line_count; 


  char cwd[256];
  char *fname = "python_fragment_longname_not_to_make_a_collision.py";
  ...
  buf_write(curbuf_dummy, ffname, (char_u*)fname, 1, pos_n.lnum,
    eap, 0, 0, TRUE, FALSE);
  ...
  //reset the current namespace of IPython
  send_to_child(wfd, "%reset\n");
  send_to_child(wfd, "y\n");

  //write to the child process
  FILE *fp = fopen((char*)ffname, "rb");
  char s[256];
  int isspecified = 0;
  if(numofcell == 0)
    isspecified = 1;
  int numbracket = 0;
  int istext = 0;
  for (int k = 0; k < pos_n.lnum; k++){
    if((fgets(s, 256, fp)) == NULL) return;
    ...
    if(s[0] == '#'
      ...
      && s[4] == '='
      && s[5] == '='
      && strncmp(s+6, (char*)eap->arg,strlen((char*)eap->arg)) == 0)
      isspecified = 1;

    for(int j = 0; j < 256 - 5; j++){
      if(s[j] == '\0' || isspecified == 1)
        break;
      ...
      else if(istext == 0 && s[j] == '\'')
        istext = 1;
      else if(istext == 0 && s[j] == '\"')
        istext = 2;
      else if(istext == 1 && s[j] == '\'')
        istext = 0;
      else if(istext == 2 && s[j] == '\"')
        istext = 0;
